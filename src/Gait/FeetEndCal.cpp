...
Vec3 FeetEndCal::calFootPos(int legID, Vec2 vxyGoalGlobal, float dYawGoal, float phase){
    ...
    _nextYaw = _dYaw*(1-phase)*_Tswing + _dYaw*_Tstance/2 + _kyaw*(dYawGoal - _dYaw); // 最后一项应该是 _dYaw - dYawGoal吧？
    ...
    _footPos = _est->getPosition() + _nextStep;  // 为什么是base坐标+平移与转动的变量？ 不应该是大腿HIP的世界坐标吗？
    _footPos(2) = 0.0;
